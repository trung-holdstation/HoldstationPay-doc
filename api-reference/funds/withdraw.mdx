---
title: "Create Withdrawal from Fund"
api: "POST https://pay-api.holdstation.com/partners/orders/withdraw"
description: "Create a withdrawal/off-ramp order with a confirmed quote. No VND is sent until the order is confirmed."
---
...
<ParamField body="kyc_id" type="string">
  Partner customer KYC id. **Required** when the `partner_kyc` feature flag is enabled for your account — the order is rejected if this is missing or does not match a registered KYC record. See [Partner Customer KYC](/guides/partner-customer-kyc/overview).
</ParamField>
