...
description: "Create a withdrawal/off-ramp order that debits a partner fund and pays out to a bank account."
---

Create a withdrawal/off-ramp order that debits a partner fund balance and pays out to a bank account via VND fiat transfer. Use the `idempotency_key` to ensure the withdrawal request is not processed more than once.
