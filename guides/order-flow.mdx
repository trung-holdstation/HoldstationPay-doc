...
Holdstation Pay uses specific order states and processing states to track the status of each order. When creating an order, you can use the `callback` parameter to receive real-time updates about order status changes via the [Orders Webhook](/guides/webhooks/orders-webhook).
...
| **Rejected**   | 8     | Order was rejected by an operator after manual review                                                |
...
stateDiagram-v2
    ...
    P --> RJ: Rejected by operator after manual review
    C --> [*]
    F --> [*]
    CL --> [*]
    RJ --> [*]
...
| Need To Review      | 17    | Fiat received; sender name needs operator review (order state stays Processing) |
| Review Rejected     | 18    | Operator rejected the order after review (order state becomes Rejected)         |
...
For two-step withdraw orders created via the [Prefunding Offramp Flow](/guides/partner-prefunding), the order moves through a dedicated processing-state set after the partner calls confirm-withdraw.
