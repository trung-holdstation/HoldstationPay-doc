...
| `pay.partner.fund-balance-changed`  | Partner-level webhook (see section below)     | Triggered when a partner fund balance changes (deposit, withdraw, compensate) |

## Fund Balance Change Event

When a partner's prefund balance changes due to an on-chain deposit, an offramp withdrawal, or a manual compensation, Holdstation Pay sends a `pay.partner.fund-balance-changed` event to the URL configured via [Set Partner Webhook URL](/api-reference/partners/set-webhook-url).
...
The `X-HSPay-Event-Signature` header is computed exactly the same way as for order webhooks — verify it with your **Webhook Checksum Key** (see [Webhook Verification](#webhook-verification) below).
