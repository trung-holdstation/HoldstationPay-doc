...
stateDiagram-v2
    ...
    state "Need To Review = 17" as NR
    state "Review Rejected = 18" as RR

    [*] --> FP
    FP --> FC: Bank notify successful result
    FP --> CL: Order expired or closed manually
    FP --> FF: Bank notify failure result
    FP --> NR: Sender name does not match KYC
    NR --> FC: Operator accepts
    NR --> RR: Operator rejects
    RR --> RJ
    FF --> F
    FC --> CP: Transfer USDT
